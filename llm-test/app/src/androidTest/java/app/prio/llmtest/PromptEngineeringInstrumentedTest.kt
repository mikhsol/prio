...
import app.prio.llmtest.benchmark.ExtendedTestDataset
import app.prio.llmtest.benchmark.PromptBuilder
import app.prio.llmtest.benchmark.PromptEngineeringBenchmark
import app.prio.llmtest.benchmark.PromptStrategy
    ...
    fun setup() {
        runBlocking {
            ...
            if (modelFile.exists() && modelFile.length() == EXPECTED_MODEL_SIZE) {
                ...
                Log.w(TAG, "Model not found or wrong size. Copy using: adb shell 'cat /data/local/tmp/Phi-3-mini-4k-instruct-q4.gguf | run-as app.prio.llmtest sh -c \"cat > /data/data/app.prio.llmtest/files/model.gguf\"'")
            }
        }
    }
